    ...
    elseif ($fieldName == 'external_identifier') {
      ...
      if (!$contID) {
        if (in_array(get_class($form), [
          ...
          $contID = $form->getVar('_contactID');
          if (empty($contID)) {
            // check custom variable have contact id, if yes, this will skip additional dedupe call
            $contID = $form->get('dedupe_cid');
          }
          $onlineForm = TRUE;
        }
        else {
      ...
      // PATCH STARTED
      // MATCH contact using External Identifier along with other field used in Unsupervised Dedupe Rule.
      // This will insure that same contact id get updated instead of showing Validation Error.
      // This Patch only work on Standalone Profile form (atleast tested on standalone form)
      if (!$contID && ($gid = $form->getVar('_gid') || $onlineForm)) {
        // only do validation if external_identifier field have value
        $external_identifier_value = CRM_Utils_Array::value('external_identifier', $form->_submitValues);
        if (!empty($external_identifier_value)) {
          $exceptions = [];
          if ($onlineForm && !empty($form->_values)) {
            $gid = CRM_Utils_Array::value('custom_pre_id', $form->_values);
          }
          $contactType = '';
          if ($gid) {
            $contactType = CRM_Core_BAO_UFGroup::getContactType($gid);
          }
          if (!$contactType) {
            $contactType = 'Individual';
          }
          $ids = CRM_Contact_BAO_Contact::getDuplicateContacts(
            $form->_submitValues, $contactType, 'Unsupervised', $exceptions, FALSE, $form->_ruleGroupID
          );
          if ($ids) {
            if ($form->_isUpdateDupe == 1) {
              $contID = $ids[0];
              $form->setVar('_id', $contID);
              $form->set('id', $contID);
            }
            elseif ($onlineForm) {
              $contID = $ids[0];
              // assign contact id to custom variable,
              $form->set('dedupe_cid', $contID);
            }
          }
        }
      }
