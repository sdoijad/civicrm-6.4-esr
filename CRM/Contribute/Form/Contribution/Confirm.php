...
class CRM_Contribute_Form_Contribution_Confirm extends CRM_Contribute_Form_ContributionBase {
    ...
    if (is_array($membershipTypeIDs) && !empty($membershipContributionID)) {
      ...
      foreach ($membershipTypeIDs as $membershipTypeID) {
        ...
        if (empty($membershipID)) {
          // PATCH for Pending membership payment.
          // otherwise it create new membership record and existing record
          // remain pending.
          $membershipID = $this->getMembershipIDFromLineItems($membershipLineItems[$this->getPriceSetID()], $membershipTypeID);
          if (!empty($membershipID)) {
            $memParams['id'] = $membershipID;
          }
        }
   ...
   * @throws CRM_Core_Exception
   */
  public function getMembershipIDFromLineItems($linesItems, $membershipTypeID) {
    $membershipID = NULL;
    foreach ($linesItems as $linesItem) {
      if (!empty($linesItem['membership_type_id']) && $linesItem['membership_type_id'] == $membershipTypeID) {
        if ($linesItem['entity_table'] == 'civicrm_membership' && !empty($linesItem['entity_id'])) {
          $membershipID = $linesItem['entity_id'];
          break;
        }
      }
    }
