...
";
  ...
  public function getSearchSQLParts($offset = 0, $rowCount = 0, $sort = NULL,
    ...
    // Force to use 'index_sort_name' index on sort name search.
    // Normally, indexes are not used when wildcard operators are used.
    if ($this->_mode & CRM_Contact_BAO_Query::MODE_ACTIVITY) {
      foreach ($this->_params as $values) {
        $name = $values[0] ?? NULL;
        $op = $values[1] ?? NULL;
        if ($name === 'sort_name' and $op === 'LIKE') {
          $from = str_replace('FROM civicrm_contact contact_a', 'FROM civicrm_contact contact_a force index(index_sort_name)', $from);
        }
      }
    }
